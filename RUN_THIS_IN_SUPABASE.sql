...
-- This script will set up everything needed for the workout selection feature

-- STEP 1: Add missing columns and constraints (safe to run multiple times)
-- =========================================================================
...
-- Add missing unique constraint on favorite_workouts (only if it doesn't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'favorite_workouts_user_workout_unique'
  ) THEN
...
-- Add missing unique constraint on active_workout_sessions (only if it doesn't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'active_workout_sessions_user_workout_unique'
  ) THEN
...
-- STEP 3: Create/Replace Functions
-- =========================================================================
...
-- STEP 7: Add sample workout data if none exists
-- =========================================================================

-- Check if we have any workouts, if not, add some
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM workouts LIMIT 1) THEN
...
-- Update descriptions for existing workouts that don't have them
UPDATE workouts SET
  description = CASE
...
-- This should return workouts if everything is set up correctly
SELECT * FROM search_workouts();

-- Success! You should now see workouts in your app