...
import { matchDetectedFoods, type DetectedFood } from '@/lib/meal-logging/api/foods'
...
export function MealScanClient() {
  ...
  const handleAnalyze = useCallback(async (imageFile?: File) => {
    ...
    try {
      ...
      const category = result.category || 'my meal'
      ...
      const message = `I just ate ${foodList} for ${category}. Estimated ${calories} calories, ${protein}g protein.`
      ...
      for await (const chunk of stream) {
        ...
        if (chunk.food_detected && chunk.food_detected.is_food) {
          ...
          try {
            ...
            const mealData = {
              category: foodData.category || result.category || 'other',
