-- FIXED MIGRATION: Enforce that users NEVER see workouts without exercises
...
DROP VIEW IF EXISTS workouts_with_exercises CASCADE;
...
-- Grant access to authenticated users (views don't need RLS policies)
...
DROP FUNCTION IF EXISTS get_workout_details(INTEGER);
...
-- Step 5: Create a diagnostic function to identify workouts without exercises
DROP FUNCTION IF EXISTS find_empty_workouts();
...
-- Step 8: Test the dashboard function
SELECT * FROM get_dashboard_workouts(NULL) LIMIT 5;

-- Step 9: Show any workouts that would be hidden
